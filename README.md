...
Right-click an image on X/Twitter, Bluesky, or a supported Mastodon instance →
...
- **Direct media URLs** (an image opened in its own tab from any supported
  ...
  builds the full smart filename. If no such tab is open: on Bluesky the
  author is still recovered via the public PLC directory lookup of the DID in
  the URL; elsewhere the name falls back to the site's file hash, since bare
  media URLs carry no author info and Mastodon has no public reverse lookup.
